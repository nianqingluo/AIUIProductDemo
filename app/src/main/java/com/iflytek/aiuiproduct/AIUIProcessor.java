...
//
					mPlayController.playTone("", WAKE_UP_TONES[new Random().nextInt(WAKE_UP_TONES.length)], null); //最后一个参数表示mPlayController播放完成的接口回调，
				...
				try {
					...
					if (content.has("cnt_id")) {
						...
						if ("nlp".equals(sub)) {
							...
							DebugLog.LogD(TAG,"在线语义cntJson:"+ cntJson.toString());//added by luobin

							if (4 == (Integer) (result.get("rc")) && ((String)result.get("text")).matches(".*介绍.*(你|(自己)|(公司)).*")){

								mPlayController.playText(""," 深圳市佳都实业发展有限公司（简称“佳都”）成立于2003年，国家级高新技术企业，是一家研发生产跨界营销智能终端、屏媒服务型机器人的专业厂商。" +
									"公司积累十余载，实力雄厚。目前旗下产品包括两大系列。一是佳都品牌旗下的跨界营销智能终端系列：包括触摸互动一体机、智能云屏、多媒体广告机等产品。二是“嘟嘟媒人”品牌旗下的屏媒服务型机器人系列：包括餐厅服务机器人、智媒体服务机器人等产品.");
							}

							if ("chat".equals(result.get("service"))){
								JSONObject answer = (JSONObject) result.get("answer");
								String topic = (String) answer.get("topic");

								if ("chat_aiui个性化_你是谁".equals(topic) && ((String)result.get("text")).matches(".*((你.*名字)|(你是谁)).*")){
									//问你的名字或者你是谁
									mPlayController.playText("","我是佳都的机器人嘟嘟");
									return;
								}
							}
							//在线语义结果
							...
							// 处理离线语法结果
							DebugLog.LogD(TAG,"离线语法cntJson:"+ cntJson.toString());//added by luobin
							mAsrHandler.handleResult(result);
						}
						else if ("iat".equals(sub)){ //added by luobin
							//处理听写结果
							DebugLog.LogD(TAG,"听写结果cntJson:"+ cntJson.toString());//added by luobin
